...
pub enum DigestItem<Hash> {
	/// 是对该块中每一条交易执行变更的证明，仅仅在该链 genesis 的时候决定是否启用。默认不启用。
	...
	///
	/// 部分PoS算法必须，在 Aura 和 Babe 中用于记录轮到某个出块者出块的证明，如Aura中的slot，Babe中还会记录VRF证明.
	PreRuntime(ConsensusEngineId, Vec<u8>),

	/// 部分PoS算法必须，如 Aura/Babe 中使用这个记录每个epoch验证者切换的列表。
	Consensus(ConsensusEngineId, Vec<u8>),

	/// 所有PoS算法必须，需要出块者使用的自己的私钥对区块进行签名，表明这个区块是由自己产出而不是别人产出。
...
/// `DigestItem` 的编码 Encode/Decode
///
/// We need an explicit control, because final runtimes are encoding their own
